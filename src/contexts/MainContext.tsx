...
const ParkProvider = ({ children }: ParkProviderType) => {
  const [{ userName, alerts , gates,isLoading,cameras,unprocessedData,vehicleType,organizations}, dispatch] = useReducer(reducer,initialStateData );
  ...
  useEffect(() => {
    ...
    const fetchUnprocessedData = async()=>{
      ...
      try {
        dispatch({type:'loading'})
        const response = await axios.get(`${BASE_URL}/transactions/unprocessed`,{
          headers:authHeader(token)
        })
    ...
    const fetchVehicleTypes = async()=>{
      ...
      try {
        dispatch({type:'loading'})
        const response = await axios.get(`${BASE_URL}/configs/rfid-ac/vehicle-types`,{
          headers:authHeader(token)
        })
  ...
  return (
    <ParkZeusContext.Provider value={{ userName, alerts ,gates,isLoading,cameras,unprocessedData ,vehicleType,organizations}}>
      {children}
